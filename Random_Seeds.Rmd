...
This is important as it means that in practice it is very hard to keep track of the internal state of the PRNG. Say you have two analysis scripts and you want some key algorithms inside to use the same pseudorandom numbers (for example you might want your tSNE plots to be reproducible). It is not sufficient to set the same random seed at the top of the script as it is all to easy too inadvertantly create a divergence in the hidden state of the PRNG between the two scripts. Therefore it is best to reseed the random number generator just the relevant code and to ensure that any intervening steps are identical.
