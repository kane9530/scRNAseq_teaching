...
```{r, graph-different-k}
# More resolved.
g.6 <- buildSNNGraph(sce.pbmc, k=8, use.dimred = 'PCA')
clust.6 <- igraph::cluster_walktrap(g.8)$membership
colLabels(sce.pbmc) <- factor(clust.6)
graph.clust.6 <- plotReducedDim(sce.pbmc, "TSNE", colour_by="label") +
  ggtitle("6 nearest neighbours")
...
multiplot(graph.clust.6,graph.clust.50,cols=2)
```
