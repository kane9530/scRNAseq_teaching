...
There are many good [explanations](https://blog.easysol.net/machine-learning-algorithms-3) and [visualisations](https://www.naftaliharris.com/blog/visualizing-k-means-clustering/) of how the k-means algorithm works online so I won't go into the details here. The most important algorithm that the k-means algorithm is the number of centers, or clusters, you use. Once this value is chosen an iterative algorithm is used to assign each point to a cluster in such a way that minimises the sum of the square distances between each point and the average position of all of the points which are assigned to that cluster.
